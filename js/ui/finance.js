...
ARCHIVO: js/ui/finance.js (Motor de Caja, Movimientos, Abonos y Cartera)
================================================================================
*/
...
function guardarEdicionMovimiento() {
    ...
    window.callAPI('editarMovimiento', payload).then(r => { 
        if(!r.exito) { alert("Error al editar: " + r.error); if(window.loadData) window.loadData(true); } 
    });
}
...
function procesarRefinanciamiento() {
    ...
    window.callAPI('refinanciarDeuda', d).then(r => { if(!r.exito) { if(window.loadData) window.loadData(true); } });
}
...
function castigarDeuda(id, nombre) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            window.callAPI('castigarCartera', {idVenta: id}).then(r => { if(!r.exito) { if(window.loadData) window.loadData(true); } });
        }
    });
}
...
function doAbono(){
    ...
    window.D.historial.unshift({ desc: "Abono: " + cli, tipo: "abono", monto: abonoNum, fecha: fechaVal || new Date().toISOString().split('T')[0], _originalIndex: window.D.historial.length, saldo: window.D.metricas.saldo });
...
function renderPed(){ 
    ...
    (window.D.ped || []).forEach(p=>{ 
        ...
        var controls = `<div class="d-flex gap-2 mt-2"><button class="btn btn-sm btn-outline-secondary flex-fill" onclick='openEditPed(${JSON.stringify(p)})'>✏️</button><button class="btn btn-sm btn-outline-danger flex-fill" onclick="delPed('${p.id}')">🗑️</button>${isPend ? `<button class="btn btn-sm btn-outline-success flex-fill" onclick="comprarPedido('${p.id}', '${p.prod.replace(/'/g, "\\'")}')">✅</button>` : ''}</div>`; 
...
function savePed(){ 
    ...
    var d = { user: window.currentUserAlias, prod: p, prov: document.getElementById('pe-prov').value, costoEst: document.getElementById('pe-costo').value, notas: document.getElementById('pe-nota').value }; 
    window.callAPI('guardarPedido', d).then(()=>{ if(window.loadData) window.loadData(true); }); 
...
function guardarEdicionPed() { 
    ...
    var d = { id: window.pedEditId, prod: document.getElementById('ed-ped-prod').value, prov: document.getElementById('ed-ped-prov').value, costoEst: document.getElementById('ed-ped-costo').value, notas: document.getElementById('ed-ped-nota').value }; 
    if(window.myModalEditPed) window.myModalEditPed.hide(); 
    if(window.showToast) window.showToast("Editando pedido...", "info"); 
    window.callAPI('editarPedido', d).then(r => { if(r.exito) { if(window.loadData) window.loadData(true); } else { alert(r.error); } }); 
}
    ...
    Swal.fire({ 
        ...
        if (result.isConfirmed) { 
            ...
            window.callAPI('procesarCompraPedido', { idPedido: id, costoReal: result.value }).then(r => { 
                if(r.exito) { 
                    Swal.fire('¡Éxito!', 'Gasto registrado e inventario actualizado.', 'success').then(() => { if(window.loadData) window.loadData(true); }); 
                } else { 
                    alert(r.error); 
                } 
            }); 
