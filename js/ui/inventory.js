...
function renderProvs() {
    ...
    (window.D.proveedores || []).forEach(p => {
        ...
        c.innerHTML += `<div class="prov-item"><div><strong>${p.nombre}</strong><br><small class="text-muted">${p.tel||'Sin numero'}</small></div><div class="d-flex gap-2">${btn}<button class="btn btn-sm btn-light border" onclick="window.editarProv('${p.nombre}')">✏️</button></div></div>`;
    });
}
...
function renderWeb() {
    ...
    lista.slice(0, 50).forEach(p => {
        ...
        c.innerHTML += `<div class="card-k"><div class="d-flex justify-content-between align-items-center"><div class="d-flex gap-2 align-items-center">${img}<div><strong>${p.nombre}</strong><br><small class="badge bg-primary">${p.catWeb}</small> <small class="text-muted">| ${window.COP.format(p.publico)}</small></div></div><button class="btn btn-sm btn-outline-danger fw-bold" onclick="window.toggleWebStatus('${p.id}')">Desactivar</button></div></div>`;
    });
}
...
function renderInv(){ 
    ...
    lista.slice(0, 50).forEach(p=>{
        ...
        div.innerHTML = `<div class="cat-img-box">${imgHtml}<div class="btn-edit-float" onclick="window.prepararEdicion('${p.id}')"><i class="fas fa-pencil-alt"></i></div></div><div class="cat-body"><div class="cat-title text-truncate" style="white-space: normal; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">${p.nombre}</div><div class="cat-price">${precioDisplay}</div><small class="text-muted" style="font-size:0.7rem;">Costo: ${window.COP.format(p.costo)}</small></div><div class="cat-actions"><div class="btn-copy-mini" onclick="window.copyingDato('${p.id}')" title="Copiar ID">ID</div><div class="btn-copy-mini" onclick="window.copyingDato('${p.nombre.replace(/'/g, "\\'")}')" title="Copiar Nombre">Nom</div><div class="btn-copy-mini" onclick="window.copyingDato('${p.publico}')" title="Copiar Precio">$$</div>${btnAddCart}${btnShareNative}</div>`;
        c.appendChild(div);
    }); 
}
...
function guardarCambiosAvanzado(){
   ...
   var newVal = { id: window.prodEdit.id, nombre: document.getElementById('inp-edit-nombre').value, cat: document.getElementById('inp-edit-categoria').value, prov: document.getElementById('inp-edit-proveedor').value.toUpperCase().trim(), costo: parseFloat(document.getElementById('inp-edit-costo').value), publico: parseFloat(document.getElementById('inp-edit-publico').value), desc: document.getElementById('inp-edit-desc').value, foto: window.prodEdit.foto || "", enWeb: document.getElementById('inp-edit-web').checked, catWeb: document.getElementById('inp-edit-cat-web').value };
   var f = document.getElementById('inp-file-foto').files[0];
   var promise = Promise.resolve(null);
   if(f) { promise = compressImage(f); }
   promise.then(b64 => {
       var idx = window.D.inv.findIndex(x => x.id === window.prodEdit.id);
       if(idx > -1) { if(b64) { newVal.foto = b64; } window.D.inv[idx] = newVal; }
       renderInv(); 
       ...
       var payload = { id: newVal.id, nombre: newVal.nombre, categoria: newVal.cat, proveedor: newVal.prov, costo: newVal.costo, publico: newVal.publico, descripcion: newVal.desc, urlExistente: window.prodEdit.foto || "", enWeb: newVal.enWeb, catWeb: newVal.catWeb };
...
function generarIDAuto(){ 
    ...
    if(c) document.getElementById('new-id').value=c.substring(0,3).toUpperCase()+'-'+Math.floor(Math.random()*9999); 
}
...
function procesarWA(){ 
    var p=document.getElementById('wa-prov').value,c=document.getElementById('wa-cat').value,t=document.getElementById('wa-text').value; 
