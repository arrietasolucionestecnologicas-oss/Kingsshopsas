...
function updateStatus() {
    ...
    if(navigator.onLine) {
        if(el) { el.className = 'status-sync'; el.innerText = '🟢 Conectado'; el.style.display = 'block'; }
        processQueue();
...
async function callAPI(action, data = null) {
    ...
    try {
        // CORRECCIÓN CORS: Enviamos como text/plain para evitar Preflight OPTIONS
...
async function processQueue() {
    ...
    for (const item of items) {
        ...
        } catch (e) { console.log("Cola pendiente..."); }
    }
}
...
window.onload = function() {
  ...
  if(tplEl) {
      document.getElementById('desktop-cart-container').innerHTML = tplEl.innerHTML;
      document.getElementById('mobile-cart').innerHTML = tplEl.innerHTML;
      document.querySelectorAll('#c-inicial').forEach(el => { el.removeAttribute('disabled'); el.style.background = '#fff'; el.oninput = calcCart; });
  }
  updateStatus();
  loadData();
};
...
async function loadData() {
    ...
    if (localData) { renderData(localData); if(loader) loader.style.display='none'; }
    ...
    if (navigator.onLine) {
        try {
            ...
            if (res && res.inventario) {
                ...
                renderData(res);
                showToast("Datos actualizados de la nube", "info");
            }
        } catch (e) { console.log("Usando versión local."); }
    }
...
function renderData(res) {
    ...
    if(res.metricas) {
        const uDisp = document.getElementById('user-display'); if(uDisp) uDisp.innerText = res.user || "Usuario";
        const balC = document.getElementById('bal-caja'); if(balC) balC.innerText = COP.format(res.metricas.saldo||0);
        const balV = document.getElementById('bal-ventas'); if(balV) balV.innerText = COP.format(res.metricas.ventaMes||0);
        const balG = document.getElementById('bal-ganancia'); if(balG) balG.innerText = COP.format(res.metricas.gananciaMes||0);
    }
    
    var provSelect = document.getElementById('filter-prov');
    if(provSelect) {
        provSelect.innerHTML = '<option value="">Todos</option>';
        D.proveedores.forEach(p => { provSelect.innerHTML += `<option value="${p.nombre}">${p.nombre}</option>`; });
    }
    
    renderPos(); renderInv(); renderWeb(); renderFin(); renderPed(); renderProvs(); renderCartera();
    ...
    dlp.forEach(list => { list.innerHTML = ''; (D.inv || []).forEach(p => { var o=document.createElement('option'); o.value=p.nombre; list.appendChild(o); }); });

    var editCat = document.getElementById('inp-edit-categoria');
    if(editCat){ editCat.innerHTML = ''; (res.categorias || []).forEach(c => { var o = document.createElement('option'); o.value = c; o.text = c; editCat.appendChild(o); }); }
    updateGastosSelect();
}

function updateGastosSelect() {
    var sg = document.getElementById('g-vinculo');
    if(sg && D.ultimasVentas) {
...
function renderPos(){
  var searchInp = document.getElementById('pos-search'); if(!searchInp) return;
  ...
  // LOGICA POS: SI NO HAY BÚSQUEDA, MUESTRA PLACEHOLDER (LIMPIEZA)
  if(!q) { if(placeholder) placeholder.style.display = 'block'; return; }
  ...
  res.slice(0,20).forEach(p => {
    ...
    div.innerHTML = `<div class="info"><div class="name">${p.nombre}</div><div class="meta">${descCorto}</div></div><div class="price">${precioDisplay}</div>`;
    c.appendChild(div);
  });
}
...
function updateCartUI() {
   ...
   if(count === 0) document.getElementById('mobile-cart').classList.remove('visible');
...
function toggleManual() {
    ...
    if(isManual) { inpTotal.style.display = 'inline-block'; txtTotal.style.display = 'none'; inpUtil.disabled = true; setTimeout(() => { inpTotal.focus(); }, 100); } else { inpTotal.style.display = 'none'; txtTotal.style.display = 'inline-block'; inpUtil.disabled = false; }
    calcCart();
}
...
function calcCart() {
   ...
   var base = 0;
   if (isManual) { var manualVal = parseFloat(parent.querySelector('#res-cont-input').value); base = isNaN(manualVal) ? 0 : manualVal; } 
   else { base = CART.reduce((acc, item) => { if(item.publico > 0) return acc + item.publico; return acc + (item.costo * (1 + util/100)); }, 0); if(conIva) base = base * 1.19; document.querySelectorAll('#res-cont').forEach(e => e.innerText = COP.format(Math.round(base))); document.querySelectorAll('#res-cont-input').forEach(e => e.value = Math.round(base)); }
   ...
   if(metodo === "Crédito") {
       ...
       var inicial = 0;
       if(isTypingInicial) { inicial = parseFloat(parent.querySelector('#c-inicial').value); if(isNaN(inicial)) inicial = 0; } else { inicial = base * 0.30; }
       calculatedValues.inicial = inicial;
       var saldoRestante = base - inicial; if(saldoRestante < 0) saldoRestante = 0;
       var saldoConInteres = saldoRestante * (1 + inter/100); 
       var valorCuota = saldoConInteres / cuotas;
       rowCred.forEach(e => { e.style.display = 'block'; e.querySelector('#res-ini').innerText = COP.format(Math.round(inicial)); e.querySelector('#res-cuota-val').innerText = COP.format(Math.round(valorCuota)); e.querySelector('#res-cuota-txt').innerText = `x ${cuotas} cuotas`; });
       inpInicial.forEach(e => { if(!isTypingInicial) e.value = Math.round(inicial); e.style.display='block'; e.disabled = false; e.style.background = '#fff'; });
...
function finalizarVenta() {
   ...
   var d = { items: itemsData, cliente: cli, metodo: metodo, inicial: (metodo === 'Crédito') ? calculatedValues.inicial : 0, vendedor: D.user || "Offline User", fechaPersonalizada: fechaVal };
   var loader = document.getElementById('loader'); if(loader) loader.style.display='flex';
   callAPI('procesarVentaCarrito', d).then(r => { 
       if(r.exito) { 
           if(r.offline) { alert("Guardado OFFLINE. Se subirá al tener conexión."); location.reload(); } else { location.reload(); }
       } else { alert(r.error); if(loader) loader.style.display='none'; } 
   });
}
...
function renderCartera() {
    ...
    if(!D.deudores || D.deudores.length === 0) { c.innerHTML = '<div class="text-center text-muted p-5">👏 Excelente, no hay deudas pendientes.</div>'; } 
    else { D.deudores.forEach(d => { totalDeuda += d.saldo; var fechaTxt = d.fechaLimite ? `<small class="text-muted"><i class="far fa-calendar-alt"></i> Vence: ${d.fechaLimite}</small>` : '<small class="text-muted">Sin fecha</small>'; c.innerHTML += `<div class="card-k card-debt"><div class="d-flex justify-content-between align-items-center"><div><h6 class="fw-bold mb-1">${d.cliente}</h6><small class="text-muted d-block text-truncate" style="max-width:150px;">${d.producto}</small>${fechaTxt}</div><div class="text-end"><h5 class="fw-bold text-danger m-0">${COP.format(d.saldo)}</h5><div class="mt-1"><span class="badge-debt">Pendiente</span></div></div></div></div>`; }); }
...
function renderInv(){ 
    var searchInp = document.getElementById('inv-search'); if(!searchInp) return;
    var q = searchInp.value.toLowerCase().trim();
...
function guardarCambiosAvanzado(){
   ...
   promise.then(b64 => {
       ...
       if(idx > -1) { if(b64) { if(b64) newVal.foto = b64; } D.inv[idx] = newVal; }
...
function renderFin(){ 
  ...
  var h=document.getElementById('hist-list'); if(!h) return;
  h.innerHTML=''; var dataHist = D.historial || []; 
