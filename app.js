...
function loadData(){
  callAPI('obtenerDatosCompletos').then(res => {
    ...
    D.ultimasVentas = res.ultimasVentas || []; // Guardar ventas para el select
    ...
    // MEJORA 1: POBLAR SELECT DE VINCULO GASTOS
...
function toggleManual() {
    ...
    if(isManual) {
        ...
        inpUtil.disabled = true; // Desactivar utilidad porque el precio es libre
    } else {
...
function calcCart() {
   if(CART.length===0) { 
       document.querySelectorAll('#res-cont').forEach(e => e.innerText = '$0'); 
       return; 
   }
   // Detectar contexto (Escritorio o Movil)
   ...
   if(metodo === "Crédito") {
       ...
       rowCred.forEach(e => {
           ...
           e.querySelector('#res-ini').innerText = '$'+Math.round(inicial).toLocaleString();
           e.querySelector('#res-cuota-val').innerText = '$'+Math.round(valorCuota).toLocaleString();
...
function finalizarVenta() {
   ...
   // Si el total es 0 (modo manual vacío), alertar
   if(calculatedValues.total <= 0) return alert("El precio de venta no puede ser 0");
   ...
   callAPI('procesarVentaCarrito', d).then(r => {
      if(r.exito) { location.reload(); } else { alert(r.error); document.getElementById('loader').style.display='none'; }
   });
}
